...
//$found_box.html( '' );
return a(b.el).hasClass("has-faces")?(a(d).remove(),a(b.el).removeClass("has-faces").html("Detect faces"),a(".face-detection-image").data("faces","").find(".face").remove(),b.save({faces:0})):a(d).faceDetection({complete:function(c){
...
console.log("error",d,a),b.update_status("Error ("+c+"): "+d)}})},show_existing:function(c,d){d=d||"normal";var e=a(b.image).width(),f=b.images.original[1]/e;c&&c.length&&a.each(c,function(a,c){""!==c&&b.add_hotspot({x:c.x/f,// + ((hotspot_width/correction)/2),
...
type:"normal"},c),c.x=Math.max(0-c.width/2,Math.min(c.x,d-c.width/2)),c.y=Math.max(0-c.width/2,Math.min(c.y,e-c.width/2)),a('<div class="hotspot '+c.type+'"></div>').css({left:c.x/d*100+"%",top:c.y/e*100+"%",width:c.width/d*100+"%",paddingBottom:c.width/d*100+"%"}).attr("title","normal"===c.type?"Click to toggle on/off":"").appendTo(f).click(function(){!a(this).hasClass("face")&&f.hasClass("active")&&a(this).remove()})},
